...
def createUser(login_session):
    if login_session['email']:
        ...
        try:
            ...
            print("getUserInfo(): MultipleResultsFound")
            return None
...
def editCategory(category_name):
    if 'user_id' not in login_session:
        ...
        editedCategory = Session.query(Category).filter(
            and_(Category.user_id == login_session['user_id'],
                 Category.name == category_name)).one_or_none()
        if editedCategory is None:
            print('User not authorized to edit this category')
...
def deleteCategory(category_name):
    if 'user_id' not in login_session:
        ...
        categoryToBeDeleted = Session.query(Category).filter(
            and_(Category.user_id == login_session['user_id'],
                 Category.name == category_name)).one_or_none()
        if categoryToBeDeleted is None:
            print('User not authorized to delete this category')
...
def editPerson(category_name, person_id):
    if 'user_id' not in login_session:
        ...
        editedPerson = Session.query(Person).filter(
            and_(Person.user_id == login_session['user_id'],
                 Person.id == person_id)).one_or_none()
        if editedPerson is None:
            print('User not authorized to edit this person')
...
def deletePerson(category_name, person_id):
    if 'user_id' not in login_session:
        ...
        personToBeDeleted = Session.query(Person).filter(
            and_(Person.user_id == login_session['user_id'],
                 Person.id == person_id)).one_or_none()
        if personToBeDeleted is None:
            print('User not authorized to delete this person')
            flash("You are not authorized to delete this person")
            return redirect(
                url_for('showCategory', category_name=category_name))
        else:
            if request.method == 'POST':
